...
{
    public class ProgramDay04
    {
        public static void Run()
        {
            Console.WriteLine("ProgramDay04");
        ...
        public class GuardEvent
        {
            ...
            public GuardEvent(string line)
            {
                ...
                this.Date = DateTime.ParseExact(spaceParts[0].Substring(1) + " " + spaceParts[1].Substring(0, spaceParts[1].Length - 1), "yyyy-MM-dd HH:mm", CultureInfo.InvariantCulture);

                if (spaceParts[2] == "Guard")
                {
                    this.Number = int.Parse(spaceParts[3].Substring(1));
                    this.Type = GuardEventType.BeginShift;
                }
                else if (spaceParts[2] == "falls")
                {
                    this.Type = GuardEventType.FallsAsleep;
                }
                else if (spaceParts[2] == "wakes")
                {
                    this.Type = GuardEventType.WakesUp;
                }
            }
        }
        ...
        static List<GuardEvent> parse(List<string> values)
        {
            List<GuardEvent> guardEvents = values.Select(x => new GuardEvent(x)).ToList();

            guardEvents = guardEvents.OrderBy(x => x.Date).ToList();

            int currentGuard = 0;

            foreach (GuardEvent guardEvent in guardEvents)
            {
                if (guardEvent.Type == GuardEventType.BeginShift)
                    currentGuard = guardEvent.Number;
                else
                    guardEvent.Number = currentGuard;
            }
        ...
        public class GuardMoment
        {
            ...
            public int AsleepMostValue { get; set; }

            public int AsleepMostMinute { get; set; }

            public GuardMoment()
            {
                this.Moments = new int[60];
            }
        }
        ...
        static void execute(List<GuardEvent> guardEvents)
        {
            ...
            foreach (IGrouping<int, GuardEvent> numberGrouping in guardEvents.GroupBy(x => x.Number))
            {
                GuardMoment guardMoment = new GuardMoment();

                guardMoments.Add(numberGrouping.Key, guardMoment);

                foreach (IGrouping<DateTime, GuardEvent> dateGrouping in numberGrouping.GroupBy(x => x.Date.Date))
                {
                    int index = 0;
                    GuardEventType guardEventType = GuardEventType.WakesUp;

                    foreach (GuardEvent guardEvent in dateGrouping)
                    {
                        if (guardEventType == GuardEventType.FallsAsleep)
                        {
                            for (int i = index; i < guardEvent.Date.Minute; i++)
                            {
                                guardMoment.Moments[i] += 1;
                                guardMoment.MinutesAsleep += 1;

                                if (guardMoment.Moments[i] > guardMoment.AsleepMostValue)
                                {
                                    guardMoment.AsleepMostMinute = i;

                                    guardMoment.AsleepMostValue = guardMoment.Moments[i];
                                }
                            }
                        }

                        index = guardEvent.Date.Minute;

                        guardEventType = guardEvent.Type;
                    }
                }
            }

            Console.WriteLine("All:");
            foreach (KeyValuePair<int, GuardMoment> guardMomentPair in guardMoments)
            {
