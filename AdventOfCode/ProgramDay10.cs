...
namespace AdventOfCode
{
    public class ProgramDay10
    {
        ...
        {
            return values.Select(x => new Point(x)).ToList();
        }
        ...
        public class Point
        {
            ...
            public Point(string line)
            {
                ...
                List<int> coords = parts[1].Split(">")[0].Split(",").Select(x => int.Parse(x.Trim())).ToList();
                List<int> coordsVelocity = parts[2].Split(">")[0].Split(",").Select(x => int.Parse(x.Trim())).ToList();

                this.X = coords[0];
                this.Y = coords[1];

                this.VelocityX = coordsVelocity[0];
                this.VelocityY = coordsVelocity[1];
            }
